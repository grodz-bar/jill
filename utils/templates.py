...
#
#   Variables (use only in messages that have them):
...
#     {preset}   - filter name (filter_applied)
#
#   Custom emojis:
...
rescan_complete:
  ...
  enabled: true
rescan_metadata_failed:
  text: "found {playlists} playlists and {tracks} songs, but metadata failed for {failed} playlists"
  enabled: true
rescan_in_progress:
